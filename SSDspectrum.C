int SSDspectrum(char *filename, int xlower, int xupper){
  gStyle->SetOptStat("nmreuio");
  TCanvas *energy_spectrum=new TCanvas("energy_spectrum","energy_spectrum");
  TCanvas *time_spectrum=new TCanvas("time_spectrum", "time_spectrum");
  FILE *fin;

  TH1D *hist=new TH1D("hist","alpha spectrum", 4096, 0.5, 4096.5);
